...
def test_find_method_returns_definition_and_overrides(indexed_project: Config) -> None:
    result = find_method.find_method("Init", config=indexed_project)
    assert result["query"] == "Init"
    assert result["did_you_mean"] == []  # exact match, no suggestions
    hits = result["hits"]
...
def test_find_method_without_overrides(indexed_project: Config) -> None:
    result = find_method.find_method(
...
def test_find_method_suggests_on_typo(indexed_project: Config) -> None:
    # "Innit" should fuzzy-match "Init"
    result = find_method.find_method("Innit", config=indexed_project)
    assert result["hits"] == []
    assert "Init" in result["did_you_mean"]
